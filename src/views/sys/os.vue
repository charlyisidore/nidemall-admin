<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button v-permission="['GET /admin/storage/list']" class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">{{ $t('button.search.label') }}</el-button>
      <el-button v-permission="['POST /admin/storage/create']" class="filter-item" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('button.create.label') }}</el-button>
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-permission="['POST /admin/storage/update']" type="primary" size="mini" @click="handleUpdate(scope.row)">{{ $t('button.edit.label') }}</el-button>
          <el-button v-permission="['POST /admin/storage/delete']" type="danger" size="mini" @click="handleDelete(scope.row)">{{ $t('button.delete.label') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="updateDialogVisible" title="修改对象名称">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="updateDialogVisible = false">{{ $t('button.cancel.label') }}</el-button>
        <el-button type="primary" @click="updateData">{{ $t('button.confirm.label') }}</el-button>
      </div>
    </el-dialog>
